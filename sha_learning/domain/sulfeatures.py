...
class SystemUnderLearning:
    ...
    def process_data(self, path):
        ...
        Heavy parsing stays outside the lock; only the quick
        list/dict mutations are serialized inside.
        """
        # ① 线程并行干重活
        parsed_signals: List[SampledSignal] = self.parse_f(path)

        # ② 原子区：所有共享结构的一致性
        with self._proc_lock:
            self.signals.append(parsed_signals)

            driver_sig = [s for s in parsed_signals if s.label in self.driver]
            change_pts = self.find_chg_pts(driver_sig)

            events = [self.label_f(self.events, parsed_signals, pt.t) for pt in change_pts]
            tt = TimedTrace([pt.t for pt in change_pts], events)

            self.timed_traces.append(tt)
            self.traces.append(Trace(tt=tt))
