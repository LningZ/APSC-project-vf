...
if _raw_workers == 'auto':
     # If 'auto', use the number of available CPU cores
    MAX_WORKERS = os.cpu_count()
else:
    try:
        # Otherwise, try converting the input to an integer, with a minimum of 1
...
class Learner:
    ...
    def fill_table(self, max_workers: int = None):
        ...
        """
        This block uses a thread pool to simultaneously schedule all fill_row jobs,
        and as each one finishes it writes back the row.     
        """
        ...
        """
       Symmetric to the upper-half,
       ensuring all lower-half rows are filled in parallel.
        """
    ...
    def add_counterexample(self, counterexample: Trace):
        ...
        with ThreadPoolExecutor() as executor:
            ...
            results = executor.map(simulate_and_fill, to_simulate)   
