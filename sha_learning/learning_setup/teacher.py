...
config.sections()
_raw_workers = config['GENERAL'].get('MAX_WORKERS', 'auto').strip().lower()
if _raw_workers == 'auto':
    MAX_WORKERS = os.cpu_count()
else:
    try:
        MAX_WORKERS = max(1, int(_raw_workers))
    except ValueError:
        MAX_WORKERS = os.cpu_count()    

# Get the base random seed from config
BASE_SEED = int(config['GENERAL']['INITIAL_SEED'])

# Thread-local storage for per-thread RNG
_thread_rng = threading.local()

# Create a master SeedSequence from the base seed
_master_ss  = np.random.SeedSequence(BASE_SEED)

def rng():
    # If the current thread has no RNG yet, create one
    if not hasattr(_thread_rng, 'gen'):
        
        # Spawn a child SeedSequence (deterministic and reproducible)
        child_ss          = _master_ss.spawn(1)[0]
        
        # Create a thread-local default_rng using the child seed
        _thread_rng.gen   = np.random.default_rng(child_ss)
        
    return _thread_rng.gen
...
class Teacher:
    # Create a reentrant lock to ensure thread-safe operations
    ...
    def _register_new_distribution(self, avg, flow, metrics):
        """
        Create and register a new probability distribution within a lock,
        write it to the histogram, and return the ProbDistribution instance.
        """
        with Teacher._lock:
            new_distr = ProbDistribution(len(self.distributions[0]), {'avg': avg})
            self.add_distribution(new_distr, flow)# Register the new distribution (already inside lock, so no need to lock again inside)
            self.to_hist(metrics, new_distr.d_id) # Write distribution data to histogram (to_hist handles its own locking internally)
            return new_distr
            ...
            try:
                ...
                for distr in self.hist.copy():
                    ...
                    if CS == 'THERMO':
                        ...
                        noise1 = rng().normal(0.0, NOISE, size=len(v1))

                    #v1 = [x + noise1[i] for i, x in enumerate(v1)]
                    v1 = [x + n for x, n in zip(v1, noise1)]
                    ...
                    if CS == 'THERMO':
                        ...
                        noise2 = rng().normal(0.0, NOISE, size=len(v2))

                    
                    #v2 = [x + noise2[i] for i, x in enumerate(v2)]
                    v2 = [x + n for x, n in zip(v2, noise2)]
    ...
    def get_counterexample_SB(self, table: ObsTable):
        ...
        # Divide traces into chunks; each thread handles one chunk
        num_threads = min(MAX_WORKERS, len(traces)) 
            ...
            for trace in trace_list:
                for prefix in trace.get_prefixes():
                    ...
                    if not_closed:
                        ...
                        if not_consistent:
                            return prefix
            return None   # No counterexample found in this thread

         # Execute the trace checking in parallel
        results = []
        ...
        # Count the number of unique states visited by a trace
        def count_unique_states(trace: Trace) -> int: # Used to compute the number of unique states triggered by the trace
            ...
            for e_word in table.get_E():
                ...
                id_distr = self.ht_query(word, id_model, save=False)
                if id_model is not None and id_distr is not None:
                    seen.add((id_model, str(id_distr)))
            return len(seen)


        # Measure similarity between a trace and the existing table
        def similarity_with_table(trace: Trace, table: ObsTable):
            ...
            if not row.is_populated():
                return float('inf')  # Maximum dissimilarity
        ...
        def score(trace: Trace):
            sim_score = similarity_with_table(trace,table)     # Lower is better
            unique_states = count_unique_states(trace)   # Higher is better
            unique_events = len(set(e.symbol for e in trace.events)) # Higher is better
            length = len(trace)                          # Shorter is better
        ...
        LOGGER.info(f"Selected best CE among {len(results)} candidates: {best_ce}")
        return best_ce
